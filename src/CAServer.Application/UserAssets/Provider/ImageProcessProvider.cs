...
public class ImageProcessProvider : IImageProcessProvider, ISingletonDependency
{
    ...
    public async Task<string> UploadSvgAsync(string filename)
    {
        if (filename == null)
        {
            return "";
    ...
    private async Task<Object> testSvgUpLoadGrain()
    {
        var grain = _clusterClient.GetGrain<ISvgGrain>(Guid.NewGuid().ToString());
