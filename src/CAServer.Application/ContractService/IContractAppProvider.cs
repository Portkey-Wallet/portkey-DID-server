...
public interface IContractAppProvider
{
...
public class ContractAppProvider : IContractAppProvider, ISingletonDependency
{
    private readonly Options.ChainOptions _chainOptions;
    private readonly ContractServiceOptions _contractOptions;
    private readonly IObjectMapper _objectMapper;
    private readonly ILogger<ContractAppProvider> _logger;
    private readonly ISignatureProvider _signatureProvider;
    private readonly IIndicatorScope _indicatorScope;

    public ContractAppProvider(IOptions<Options.ChainOptions> chainOptions, 
        IOptions<ContractServiceOptions> contractOptions,
        IObjectMapper objectMapper, 
        ILogger<ContractAppProvider> logger, 
        ...
        string methodName)
    {
        try
        {
            ...
            refBlockNumber -= _contractOptions.SafeBlockHeight;
            ...
            await Task.Delay(_contractOptions.Delay);
            ...
            var times = 0;
            while ((transactionResult.Status == TransactionState.Pending || transactionResult.Status == TransactionState.NotExisted) && times < _contractOptions.RetryTimes)
            {
                times++;
                await Task.Delay(_contractOptions.RetryDelay);
    ...
    private async Task<TransactionInfoDto> ForwardTransactionToChainAsync(string chainId, string rawTransaction)
    {
         try
         {
            ...
            var times = 0;
            while ((transactionResult.Status == TransactionState.Pending || transactionResult.Status == TransactionState.NotExisted) && times < _contractOptions.CryptoBoxRetryTimes)
            {
                times++;
                await Task.Delay(_contractOptions.CryptoBoxRetryDelay);
                transactionResult = await client.GetTransactionResultAsync(result.TransactionId);
            }
